...
export default function Hero({ onSearchAndBook }: HeroProps) {
  ...
  return (
    <div id="hero-section" className="relative h-[80vh] w-full overflow-hidden flex items-center justify-center rounded-2xl shadow-xl">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white flex flex-col items-center">
        ...
        {/* Display Heading */}
        <h2 className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight mb-4 leading-tight">
        ...
        >
          ...
          <div className="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-xl bg-white/10 dark:bg-slate-950/40 border border-white/10 text-left">
            ...
            <div className="flex-1">
              ...
              >
                <option value="" className="text-slate-850">Choose category...</option>
                <optgroup label="🎩 Formal Events" className="text-slate-850">
                  {EVENT_CATEGORIES.filter(c => c.type === 'formal').map(c => (
                    <option key={c.id} value={c.id} className="text-slate-850">{c.name}</option>
                  ))}
                </optgroup>
                <optgroup label="🎉 Informal Events" className="text-slate-850">
          ...
          <div className="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-xl bg-white/10 dark:bg-slate-950/40 border border-white/10 text-left">
            ...
            <div className="flex-1">
              ...
              <select
                ...
                className="block w-full bg-transparent border-0 p-0 text-xs font-semibold text-white focus:ring-0 focus:outline-none focus:bg-slate-900 cursor-pointer"
              >
                <option value="" className="text-slate-850">Choose city...</option>
                {Object.keys(states).map((state) => (
                  <optgroup key={state} label={state} className="text-slate-850">
                    {states[state].map((city) => (
                      <option key={city.id} value={city.id} className="text-slate-850">{city.name}</option>
                    ))}
                  </optgroup>
                ))}
              </select>
